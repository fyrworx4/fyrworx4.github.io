...
This is going to be an update to [my first RvB post](https://tsnguyen.com/red-vs-blue/) and [my second RvB post](https://tsnguyen.com/red-vs-blue2/), so please check my previous post out if you haven't already.
...
Another term we must talk about are the actual ethernet ports of the ESXi servers (which were PowerEdges). The physical, ethernet ports of the ESXis are referred to as "vmnics". Each vmnic port has its own ID, starting from zero. So if we had four ethernet ports, our vmnics would be as follows:
- vmnic0
- vmnic1
- vmnic2
- vmnic3
...
(literally, those ethernet ports are called vmnics in ESXi language)
...
Each network would need to be physicaly separated with two different physical switches. However, with VLANs, you can attach multiple, logically separated networks on a single switch.
...
We also had an upgrade to our lab - instead of having individuals ESXi machines, we created an ESXi cluster with vMotion and iSCSI configured to allow for load balancing and other cool VM stuff.
...
Then I clicked on the "Add and Manage Hosts" setting for the dSwitch, selected all my ESXi hosts to add to the dSwitch, and designated vmnic0 of all my ESXisto be attached to the dSwitch uplinks. After this step, there were extra settings for VMkernels and all that jazz but that's outside the scope of this post.
...
This would allow a user to create a port group with a unique VLAN ID, and be able to have VMs on that port group communicate with each other regardless of which ESXi the VM belonged on. Things like creating VMs, vMotion, and load balancing cause VMs to be automatically placed and migrated in different ESXis, making this configuration an ideal way to abstract the networking on the individual ESXi level.
